...
const DuckGooseGame = () => {
	...
	return (
    <div className="h-screen w-screen overflow-hidden relative bg-blue-50 text-black">
      ...
      <div className="absolute top-4 left-4 space-y-2">
        ...
        <div className="text-lg font-medium">Score: {Math.floor(score)}</div>
      </div>

      {countdown > 0 && (
        <div className="absolute top-4 right-4 text-2xl font-bold text-red-600">
          {countdown}s
        </div>
      )}
      ...
      {gameState === "gameover" && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-center bg-white p-8 rounded-lg shadow-lg">
          ...
          <p className="text-xl mb-4">Final Score: {Math.floor(score)}</p>
          <div className="mb-6">
